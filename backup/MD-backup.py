...
elevation, lon_grid, lat_grid = np.load('dem_data.npy')[:]
obs = np.load('obs.npy').item()
bm = pickle.load(open(basemap_path,'rb'))   # load here the above pickle 
landmask = np.load('landmask_-128.8_-121.0_48.2_51.0.npy')
...
print 'Initiating Mother-Daughter routine...'
...
class MDtile:
	def __init__(self, demPoints, demWeights, mother_idx, HA_idx):
...
for nStn in range(num_stn):		
	...
	first_tile = MDtile(demPoints, demWeights, mother_idx, HA_idx)						#initate an MD tile around the mother
	...
	while len(update_ids) > 0:									#as long as there are points to update, continue loop
		...
		for nDaughter, daughter in enumerate(update_ids): 		#iterate through modified points
			...
			daughter_tile = MDtile(demPoints, demWeights, mother_idx, HA_idx) 			#repeat MD as above
...
#if a weights file already exists for current configuration append the dictionary with new stations
if os.path.isfile(weights_filename+'.npy') and os.path.isfile(id_filename+'.npy'):
	print 'Appending existing weights file'
	md_weights_old = np.load(weights_filename+'.npy')
	md_id_old = np.load('weights_id_%s-%s-%s-%s.npy' %params[:])
	md_weights_allstn = numpy.concatenate(md_weights_old,md_weights_allstn)
	md_id = numpy.concatenate(md_id_old,obs['id'][:])
else:
	md_id = obs['id'][:]
np.save(weights_filename + '.npy', md_weights_allstn)
np.save(id_filename + '.npy', md_id)


print('MD weights saved in: ' + weights_filename + '.npy')
print('MD IDs saved in: ' + id_filename + '.npy')
...
#consider MPI
#figure out station with huge elevation gap
