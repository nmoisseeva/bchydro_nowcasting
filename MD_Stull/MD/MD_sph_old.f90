...
!**********************************************************
!* author: Xingxiu Deng, Jan. 17, 2003, last modified, Mar., 2006
!*-objective:
...
!*    HAlatrot/HAlonrot, and latrot/lonrot have to be used for comparing relative
...
!*                                  index_HAx,index_HAy,rmin)
          CALL find_index_HA_sph(ipt,jpt,HAx(ista),HAy(ista),Z_HA(ista), &
                                  HAxyz_rot_8(ista),ni,nj,xgr,ygr,Zdaughter,   &
                                  xyzgr_rot_8,index_HAx,index_HAy,rmin)
...
! sharing factor if the point to the north-east is its mother
         ...
         IF ( i1 .GE. 1 .and. i1 .LE. ni            &
          ...
          CALL mother(ista,z_HA,n_HA,ni,nj,Smother,i1,j1,s,1,Zdaughter,i,j, &
                      a,b,zref1,zref2)
          CALL distance_sph(r,1,i1,j1,i,j,xyzgr_rot_8,ni,nj)
         ELSE
          s(1) = 0.
          r(1) = 0.
          imother(1) = i
...
! sharing factor if the point to the east is its mother
         ...
         IF ( i1 .GE. 1 .and. i1 .LE. ni            &
          ...
          CALL mother(ista,z_HA,n_HA,ni,nj,Smother,i1,j1,s,2,Zdaughter,i,j, &
                      a,b,zref1,zref2)
          CALL distance_sph(r,2,i1,j1,i,j,xyzgr_rot_8,ni,nj)
         ELSE
          s(2) = 0.
          r(2) = 0.
          imother(2) = i
...
! sharing factor if the point to the south-east is its mother
         ...
         IF ( i1 .GE. 1 .and. i1 .LE. ni            &
          ...
          CALL mother(ista,z_HA,n_HA,ni,nj,Smother,i1,j1,s,3,Zdaughter,i,j, &
                      a,b,zref1,zref2)
          CALL distance_sph(r,3,i1,j1,i,j,xyzgr_rot_8,ni,nj)
         ELSE
          s(3) = 0.
          r(3) = 0.
          imother(3) = i
...
! sharing factor if the point ot the south is its mother
         ...
         IF ( i1 .GE. 1 .and. i1 .LE. ni            &
          ...
          CALL mother(ista,z_HA,n_HA,ni,nj,Smother,i1,j1,s,4,Zdaughter,i,j, &
                      a,b,zref1,zref2)
          CALL distance_sph(r,4,i1,j1,i,j,xyzgr_rot_8,ni,nj)
         ELSE
          s(4) = 0.
          r(4) = 0.
          imother(4) = i
...
! sharing factor if the point to the south-west is its mother
         ...
         IF ( i1 .GE. 1 .and. i1 .LE. ni            &
          ...
          CALL mother(ista,z_HA,n_HA,ni,nj,Smother,i1,j1,s,5,Zdaughter,i,j, &
                      a,b,zref1,zref2)
          CALL distance_sph(r,5,i1,j1,i,j,xyzgr_rot_8,ni,nj)
         ELSE
          s(5) = 0.
          r(5) = 0.
          imother(5) = i
...
! sharing factor if the point to the west is its mother
         ...
         IF ( i1 .GE. 1 .and. i1 .LE. ni            &
          ...
          CALL mother(ista,z_HA,n_HA,ni,nj,Smother,i1,j1,s,6,Zdaughter,i,j, &
                      a,b,zref1,zref2)
          CALL distance_sph(r,6,i1,j1,i,j,xyzgr_rot_8,ni,nj)
         ELSE
          s(6) = 0.
          r(6) = 0.
          imother(6) = i
...
! sharing factor if the point to the north-west is its mother
         ...
         IF ( i1 .GE. 1 .and. i1 .LE. ni            &
          ...
          CALL mother(ista,z_HA,n_HA,ni,nj,Smother,i1,j1,s,7,Zdaughter,i,j, &
                      a,b,zref1,zref2)
          CALL distance_sph(r,7,i1,j1,i,j,xyzgr_rot_8,ni,nj)
         ELSE
          s(7) = 0.
          r(7) = 0.
          imother(7) = i
...
! sharing factor if the point to the north is its mother
         ...
         IF ( i1 .GE. 1 .and. i1 .LE. ni            &
          ...
          CALL mother(ista,z_HA,n_HA,ni,nj,Smother,i1,j1,s,8,Zdaughter,i,j, &
                      a,b,zref1,zref2)
          CALL distance_sph(r,8,i1,j1,i,j,xyzgr_rot_8,ni,nj)
         ELSE
          s(8) = 0.
          r(8) = 0.
          imother(8) = i
...
!**********************************************************
         ...
         SUBROUTINE distance_sph(r,in,im,jm,    &
                             id,jd,xyzg_8,ni,nj)
!**********************************************************
!* author: Xingxiu Deng, Apr., 2006
!*   This SUBROUTINE is to calculate distance of a grid(id,jd) 
!*   point to one of its mother (im,jm) at the surface of the sphere
!**********************************************************
         IMPLICIT NONE
...
!**********************************************************
         ...
         call cal_dist_sphere(xyzd_8, xyzm_8,r(in))
...
!         ENDIF
        ...
        SUBROUTINE find_index_HA_sph(ipt,jpt,xpt,ypt,zpt,xyzpt_8,nx,ny,   &
                                  xg,yg,zg,xyzg_8,index_xpt,index_ypt,rmin)
!**********************************************************
!* author: Xingxiu Deng, Apr., 2003
!*   This SUBROUTINE is to find a nearest grid point to the obs point
!*   (xpt,ypt) given the grid point to the south and west (ipt,jpt)
!* Modifing history
!*   Apr. 18, 2006: modified for GEM grid, now rmin is defined as great
!*                  circle distance between HA and the nearest grid point.
!*                  It is based on find_index_HA.
!*   March 05, 2003: changed to find the nearest grid point based on 
!*                   elevation difference rather than horizontal 
!*                   distance. rmin now is defined as the horizontal
!*                   distance between HA and the nearest grid point.
...
!*---------- Begin executable code ----------------------
         ...
         IF ( kz .eq. 1 ) THEN
          index_xpt = ipt
          index_ypt = jpt
          call cal_dist_sphere(xyzg_8(ipt,jpt,:),xyzpt_8,rmin)
         ELSE IF ( kz .eq. 2 ) THEN
          index_xpt = ipt
          index_ypt = jpt + 1
          call cal_dist_sphere(xyzg_8(ipt,jpt+1,:),xyzpt_8,rmin)
         ELSE IF ( kz .eq. 3 ) THEN
          index_xpt = ipt + 1
          index_ypt = jpt + 1
          call cal_dist_sphere(xyzg_8(ipt+1,jpt+1,:),xyzpt_8,rmin)
         ELSE IF ( kz .eq. 4 ) THEN
          index_xpt = ipt + 1
          index_ypt = jpt 
          call cal_dist_sphere(xyzg_8(ipt+1,jpt,:),xyzpt_8,rmin)
         ENDIF
         print*, 'rmin,index_xpt,index_ypt',rmin,index_xpt,index_ypt
         print*, 'zg', zg(index_xpt,index_ypt)
         RETURN
