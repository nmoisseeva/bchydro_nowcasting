...
#include <stdlib.h>
 ...
 int main(int argc, char *argv[] ) 
 {
	...
	float* valuesT, *valuesWS, *valuesWD, *valuesPr;
	int numT, numWS, numWD, numPr;
	int* datesT, *datesWS, *datesWD, *datesPr;
	int* timesT, *timesWS, *timesWD, *timesPr;
	...
	for(nStn=0; nStn<numStat; nStn++)
	{
		...
		numPr = get_obs_(&station,&st_date,&st_time,&end_date,&end_time,&level,varPr,&datesPr,&timesPr,&valuesPr,NULL,NULL);

		if(numT == 0 && numWS == 0 && numWD == 0 && numPr == 0)									
			{
		...
		else 
			{
				...
				fprintf(obsFile,"%s %s %s %s %s %s \n", "Date", "Time", "Temperature (degC)","Wind Speed (km/h)","Wind Dir (deg)","Hourly Precip (mm)");
				if(!obsFile) 									
				{
					printf("ERROR: could not open output file %s \n", obsFilename);
				}
				...
				{
					fprintf(obsFile, "%d %d %6.2f %6.2f %6.2f %6.2f \n" ,datesT[nObs],timesT[nObs],valuesT[nObs],valuesWS[nObs],valuesWD[nObs],valuesPr[nObs]);
				}
				fclose(obsFile);
