...
def prep_da(obs,lon_grid,lat_grid,elevation):
	...
	print('Testing observations metadata')
	...
	obs_clean = obs = {'x': obs['x'][good_pts], 'y':obs['y'][good_pts], 't':obs['t'][good_pts], 'h':obs['h'][good_pts], 'id':obs['id'][good_pts], 'rain':obs['rain'][good_pts], 'dem_idx':good_ids}
	print('......... total exluded: %s' %bad_pts)
		...
		if os.path.isfile(stn_file) \
			...
			if row_count > 1:													#make sure file is not empty
				obs_data = np.genfromtxt(stn_file, \
					dtype = [('date',int),('time','S6'),('temp',float),('wind_speed',float),('wind_dir',float),('rain',float)], skiprows=1)
				...
				pt_rain = obs_data['rain'][closest_t_idx]
				if (-50 < pt_temp < 60 and del_time[closest_t_idx] < 5):		#ensure reasonable temp and del_t range
					...
					obs_h.append(stnH)
					obs_id.append(stnID)
					obs_r.append(pt_rain)
	obs_len = len(obs_x)
	obs_t = [i+273. for i in obs_t]												#convert to Kalvin
	obs = {'x': np.array(obs_x), 'y': np.array(obs_y), 't': np.array(obs_t), 'h':np.array(obs_h), 'id':np.array(obs_id),'rain':np.array(obs_r)}
...
def subset_obs(obs, verif_frac):
	...
	obs_x, obs_y, obs_h, obs_t, obs_id, obs_rain, obs_dem = np.array(obs['x']), np.array(obs['y']),np.array(obs['h']),np.array(obs['t']),np.array(obs['id']),np.array(obs['rain']),np.array(obs['dem_idx'])
	obs_xS, obs_yS, obs_hS, obs_tS, obs_idS, obs_rainS, obs_demS = obs_x[rand_idx],obs_y[rand_idx],obs_h[rand_idx],obs_t[rand_idx],obs_id[rand_idx],obs_rain[rand_idx],obs_dem[rand_idx]
	obs_x0, obs_y0, obs_h0, obs_t0, obs_id0, obs_rain0, obs_dem0 = obs_x[test_idx],obs_y[test_idx],obs_h[test_idx],obs_t[test_idx],obs_id[test_idx],obs_rain[test_idx],obs_dem[test_idx]
	
	obsTrain = {'x': obs_xS, 'y': obs_yS, 't': obs_tS, 'h':obs_hS, 'id':obs_idS, 'rain':obs_rainS, 'dem_idx':obs_demS}
	obsTest = {'x': obs_x0, 'y': obs_y0, 't': obs_t0, 'h':obs_h0, 'id':obs_id0, 'rain':obs_rain0, 'dem_idx':obs_dem0}
...
def da_md_land(obsTrain,elevation,lon_grid,lat_grid,dem_landmask,demT,interpGamma,params,bias_mode,dist_cutoff,bm):
	...
	if bias_mode:
		for nStn in range(obs_len):
			...
			md_T[nStn,:] = (np.array(T_flat) + bias) * correction_factor
	else:
		for nStn in range(obs_len):
			correction_factor = md_fix[nStn,:]
			correction_factor[landmask_flat==0] = np.nan
			...
			md_T[nStn,:] = rawtemp[:] * correction_factor
...
# def da_md(obsTrain,elevation,lon_grid,lat_grid,demVAR,params,bias_mode,dist_cutoff):
# 	'''Mother-Daugher appreach for assimilating observations. 
...
# 		dem lon/lat grids
# 	-demVAR: 2D array
# 		-downscaled variable of interest
# 	-params: list
# 		[a,b, Z_ref1, Z_ref2] - parameters for sharing factor calculation
# 	-bias_mode: boolean
# 		1: run in bias mode (correct temperature increment), 0: correct temperature
...
# 		for nStn in range(obs_len):
# 			correction_factor = md_fix[nStn,:]
# 			bias = obsTrain['rain'][nStn] - VAR_flat[obsTrain['dem_idx'][nStn]]
...
# 			# rawtemp = obsTrain['t'][nStn] + delVAR
# 			# md_T[nStn,:] = rawtemp[:] * correction_factor
# 			md_VAR[nStn,:] = obsTrain['rain'][nStn] * correction_factor
